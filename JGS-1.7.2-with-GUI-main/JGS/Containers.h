...
public:
    ...
    {
        int32 Index;
        TArray_<ArrayType>& ITArray_;

    public:
        FBaseArrayIterator(TArray_<ArrayType>& Array)
            : ITArray_(Array)
            , Index(Array.Count)
        {
        }
        FBaseArrayIterator(TArray_<ArrayType>& Array, int32 CurrentIndex)
            : ITArray_(Array)
            , Index(CurrentIndex)
        {
        }
...
struct FString_ : public TArray_<wchar_t>
{
    inline FString_() {};

    FString_(const wchar_t* other)
    {
    ...
    FORCEINLINE explicit operator bool() const
    {
        return this->TArray_<wchar_t>::operator bool();
    }
};
...
public:
    ...
    private:
        int32 Index;
        const TBiTArray_& IteratedArray;

    public:
        FORCEINLINE const FBitIterator(const TBiTArray_& ToIterate, const int32 StartIndex) // Begin
            : IteratedArray(ToIterate)
            , Index(StartIndex)
            , FRelativeBitReference(StartIndex)
        {
        }
        FORCEINLINE const FBitIterator(const TBiTArray_& ToIterate) // End
            : IteratedArray(ToIterate)
        ...
        {
            // Thesis: Once there are more elements in the BiTArray_ than InlineData can hold it'll just allocate all of 
    ...
    public:
        FORCEINLINE FSetBitIterator(const TBiTArray_& ToIterate, int32 StartIndex)
            : FRelativeBitReference(StartIndex)
            , IteratedArray(const_cast<TBiTArray_&>(ToIterate))
        ...
        FORCEINLINE FSetBitIterator(const TBiTArray_& ToIterate)
            : FRelativeBitReference(ToIterate.NumBits)
            , IteratedArray(const_cast<TBiTArray_&>(ToIterate))
            , UnvisitedBitMask(0)
    ...
    private:
        ...
        void FindNextSetBit()
        {
            //InlineData is the first 16-bytes of TBiTArray_
...
private:
    TArray_<FSparseArrayElement> Data;
    TBiTArray_ AllocationFlags;
...
public:
    ...
    private:
        ...
        TBiTArray_::FSetBitIterator BiTArray_It;

    public:
        FORCEINLINE FBaseIterator(const TSparseArray<ArrayType>& Array, const TBiTArray_::FSetBitIterator BitIterator)
            : IteratedArray(const_cast<TSparseArray<ArrayType>&>(Array))
            , BiTArray_It(const_cast<TBiTArray_::FSetBitIterator&>(BitIterator))
        {
        }

        FORCEINLINE explicit operator bool() const
        {
            return (bool)BiTArray_It;
        }
        ...
        FORCEINLINE ArrayType& operator*()
        {
            return IteratedArray[BiTArray_It.GetIndex()].ElementData;
        }
        ...
        {
            return BiTArray_It == Other.BiTArray_It;
        }
        FORCEINLINE bool operator!=(const TSparseArray<ArrayType>::FBaseIterator& Other) const
        {
            return BiTArray_It != Other.BiTArray_It;
...
public:
    ...
    {
        return TSparseArray<ArrayType>::FBaseIterator(*this, TBiTArray_::FSetBitIterator(AllocationFlags, 0));
    }
    ...
    {
        return TSparseArray<ArrayType>::FBaseIterator(*this, TBiTArray_::FSetBitIterator(AllocationFlags));
    }
    ...
    FORCEINLINE const TArray_<FSparseArrayElement>& GetData() const
    {
        return Data;
    }
    FORCEINLINE const TBiTArray_& GetAllocationFlags() const
    {
...
public:
    ...
    {
    private:
        TMap_<KeyType, ValueType>& IteratedMap;
        TSet<ElementType>::FBaseIterator SetIt;

    public:
        FBaseIterator(TMap_<KeyType, ValueType>& InMap, TSet<ElementType>::FBaseIterator InSet)
            : IteratedMap(InMap)
            , SetIt(InSet)
        {
        }
        FORCEINLINE TMap_<KeyType, ValueType>::FBaseIterator operator++()
        {
            ++SetIt;
            return *this;
        }
        FORCEINLINE TMap_<KeyType, ValueType>::ElementType& operator*()
        {
            return *SetIt;
        }
        FORCEINLINE const TMap_<KeyType, ValueType>::ElementType& operator*() const
        {
            return *SetIt;
        }
        FORCEINLINE bool operator==(const TMap_<KeyType, ValueType>::FBaseIterator& Other) const
        {
    ...
    FORCEINLINE TMap_<KeyType, ValueType>::FBaseIterator begin()
    {
        return TMap_<KeyType, ValueType>::FBaseIterator(*this, Pairs.begin());
    }
    FORCEINLINE const TMap_<KeyType, ValueType>::FBaseIterator begin() const
    {
        return TMap_<KeyType, ValueType>::FBaseIterator(*this, Pairs.begin());
    }
    FORCEINLINE TMap_<KeyType, ValueType>::FBaseIterator end()
    {
        return TMap_<KeyType, ValueType>::FBaseIterator(*this, Pairs.end());
    }
    FORCEINLINE const TMap_<KeyType, ValueType>::FBaseIterator end() const
    {
        return TMap_<KeyType, ValueType>::FBaseIterator(*this, Pairs.end());
    }
